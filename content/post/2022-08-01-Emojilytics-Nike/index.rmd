...
Businesses care about what their customers think of them, and even more importantly they consistently strive to make their advertisements so that it hits home for their target audience. Since a vast majority of users are online on platforms such as twitter a large portion of marketing is done through social media. A key attribute of discourse on social media is the use of Emojis in content to express emotion or amplify text. Therefore not only is it important to understand how users are using Emojis to talk about companies and their products but they are a potential new avenue to explore to make marketing efforts on social media more relatable to the end user. 
...
Being a young college student I have seen a majority of Emojis which are commonly used to express, or accompany, specific expressions. Most notably, as will be seen later on, the laughing face and fire Emojis which are commonly used in story-like messages and to express that something is cool, respectively. With thousands of Emojis existing today, and being used in an estimated 20% of all tweets (16% in this dataset), not all will have signficant relationships to emotions or other characteristics of tweets but with their wide use there are a large portion which may hold significant relationships.
...
There are 7 features which held importance within this model: average total word and sentiment word intensities, negative and positive sentiment word count, total words, as well as the number of words with intensity values of -1 or 2. Surprising to me none of the emotion features had any significance in the model and I believe that is due to the fact that a majority of words carry multiple emotional connotations and therefore it causes for the overlap to make these variables insignificant. But since these features do hold importance then they must have an association with specific Emoji(s). To look at this I found the mean of each of these significant features for each Emoji in the following summary table.
...
final_sum_tbl <- tweet_sum_tbl_2[, colnames(tweet_sum_tbl_2) == "Emojis" | colnames(tweet_sum_tbl_2) %in% imp_vars] %>%
  ...
  mutate(rank = order(order(fct_relevel(Emojis, levels = c("\U0001f602","\U0001f62d", "\U0001f60d", "\U0001f6a8", "\U0001f440", "\U0001f525", "\U0001f923", "\U0001f4f2"))))) %>%
  arrange(rank) %>%
  select(Emojis, "Average Total Words" = total_words, "Average Negative Words" = negative, "Average Positive Words" = positive, "Average -1 Intensity Words" = Value.1, "Average 2 Intensity Words" = Value2, "Average Word Intensity" = avg_value, "Average Sentiment Word Intensity" = avg_int_value)

kable(final_sum_tbl,
      caption = TableHeader("What the Tweets with each Emoji Look Like")) %>%
  kable_styling(latex_options = "striped", font_size=16) %>%
  row_spec(c(justodd(1:nrow(final_sum_tbl))), background="#F8F8F8")
```

The table I have constructed above depicts the average value for tweets with each Emoji in each feature The most significant findings, of the well predicted Emojis, are that the fire and eyes Emojis have the highest average emotional intensity of holding emotional sentiment, which would be expected since a large number of users utilize these Emojis to say "look at how cool, or fire, these are." This is also confirmed with these Emojis also being tied for the lowest number of negative sentiment words on average. Another surprising finding is that, while not predicted well in the model, the heart eyes Emoji is tied for the highest average number of positive words and has the most positive average word intensity value. This associates this Emoji with higher intensity positive sentiment meaning that users are also using this Emoji to show their love for Nike. This relationship may be significant and just not predicted well because it occurs at a much lower rate than the Emojis predicted well in the model. This may just be due to the mix of tweets selected and more research should be done to confirm if this relationship does exist.

After running this model I wanted to see how it would perform on a fresh set of tweets which were pulled a month after the original set. 
...
Overall the same trend of predicting the same five of the eight Emojis remained the same but, the accuracy and Kappa values of the model dropped significantly for these tweets, only correctly predicting 31% of the tweets and performing better than random chance by only 18%. This much lower performance may be due to the quickly changing Emoji use habits of users over time. Therefore since habits are changing constantly to keep this model up to date it must be re-trained seasonally or in some other cyclical manner. But, these Emoji relationships are significant to Nike, in this instance, because they would benefit from utilizing Emojis in its marketing online to communicate with the users' in their own online language. They could utilize emojis such as the laughing Emoji and police lights in story-like messages to be more relatable and the fire and eyes Emojis to convey positive sentiment or that a product is "fire." While including Emojis may not directly lead to increased sales from higher engagement it will allow for Nike to converse with their potential customers in more like they talk about Nike and its products and further its relationship with current and potential customers.
